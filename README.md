...
- 3D Tiles(포인트 클라우드) 레이어: 목록에서 우클릭 → **설정**을 펼치면 포인트 크기, 고도 offset(m), 색상 표현(원본 RGB·고도별·강도), 점 보간을 조절. 값을 바꾸면 타일을 다시 읽어 바로 반영된다.
  - **지형에 맞추기**: tileset.json의 바닥 높이와 그 지점 지형 높이를 비교해 고도 offset을 자동으로 맞춘다.
- 마지막으로 보던 위치를 기억해 다시 켜면 그 자리에서 시작한다(초기화 버튼을 누르면 기본 위치로).
- 레이어 목록:
...
- 분석 패널 · 지형 렌더링:
  ...
  - **부드러운 색 (DEM 영상으로 덮기)**: 드롭한 DEM에서 연속 그라데이션 + 힐셰이드(음영 0~10×) 영상 타일을 만들어 지형 위에 덮음. 엔진 색상의 32색 띠 제한을 우회해 ArcGIS 같은 매끈한 표현.
  - **DEM 메시 부드럽게** 체크박스로 3차 보간 업샘플링을 켜고 끄며 전/후 비교.
  - **타일 비율** 슬라이더(1~4×, 엔진 LOD 비율)로 같은 거리에서 더 상세한 타일을 가져와 메시·영상을 촘촘하게(타일 수 증가).
  - **지형 배율** 슬라이더(1~10×)로 기본 VWorld 지형과 드롭한 DEM의 높이를 함께 과장해 해저 골짜기·산의 계곡을 강조. 기본 지형은 로컬 프록시가 서버 타일의 인코딩을 풀어 배율을 곱하고, DEM은 타일 높이값에 직접 곱하므로 화면과 높이 조회가 일치.
...
- 엔진 버전: 설정 패널에서 **공식 배포 버전**([github.com/EgisCorp/XDWorld](https://github.com/EgisCorp/XDWorld)의 `Release/`)을 선택. 변경하면 앱이 다시 로딩됨. 선택한 버전의 공식 변경 내용도 함께 표시.
  - 엔진과 워커는 **항상 같은 버전**을 쓴다. 공식 안내대로 2.29.3부터 워커 파일이 바뀌었기 때문에, 엔진은 CDN에서, 워커(`XDWorldWorker.js`·`.wasm`)는 같은 버전의 공식 저장소에서 받아 캐시한다.
  - 찾는 순서: 앱 폴더 `worker/<버전>/` → 받아둔 캐시 → 워커 URL → 번들 워커. 현재 쓰는 워커가 설정 패널에 표시되고, 버전이 안 맞는 번들 워커로 떨어지면 경고색으로 보인다.
- 우주 배경: 설정 패널의 **지구본 뒤에 별 표시**(기본 켬). 줌아웃하면 지구본 뒤로 별하늘(Tycho 성도 큐브맵)이 보인다. 텍스처는 앱에 함께 들어 있어 인터넷 없이도 나온다.
- 3D Tiles(포인트 클라우드): 불러오면 **자동으로 지형 높이에 맞춰 앉는다**. LAS를 변환할 때 표고 기준이 달라 지형 아래로 잠기는 경우를 자동으로 보정한다.
  - 레이어 우클릭 → 설정에서 포인트 크기, 고도 offset, 색상 표현, 점 보간을 조절한다. 고도 offset은 엔진 제약 때문에 타일셋을 다시 읽어 적용하므로 값이 반영되기까지 몇 초 걸린다.
  - '지형에 맞추기'는 tileset.json의 바닥 높이와 그 지점 지형 높이의 차이를 offset으로 넣는다.
  - **상세도**를 올리면 한 단계씩 더 깊은 레벨의 타일까지 받아 점이 촘촘해진다(기본은 엔진 기본보다 한 단계 촘촘한 값). 올릴수록 받는 타일이 크게 늘어 느려진다.
- 전체화면: 오른쪽 아래 버튼 또는 F11.
- 새로고침 버튼: 카메라는 그대로 두고 지형·영상 타일만 다시 그림(엔진 `XDEPlanetRefresh`, 서버 타일 캐시도 비움). 앱 전체 리로드는 F5.
- 카메라:
  - 해수면 아래(수심 DEM)에서도 휠 줌이 지형까지 거리 비례로 동작하고, 지형 아래로 뚫리면 마지막 정상 위치로 복귀.
