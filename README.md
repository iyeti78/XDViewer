...
- 엔진 버전: 설정 패널에서 XDWorld 엔진 버전(latest·stable·2.20.0~2.30.1)을 선택. 변경하면 앱이 다시 로딩됨. 워커(`XDWorldWorker.js/.wasm`)는 엔진과 짝이 맞아야 하므로 버전별 워커를 `worker/<버전>/`에 두면 자동으로 함께 쓰이고, 없는 버전은 기본 워커로 동작(맞지 않을 수 있음).
