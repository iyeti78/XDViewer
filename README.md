...
- 엔진 버전: 설정 패널에서 XDWorld 엔진 버전(latest·stable·2.20.0~2.30.1)을 선택. 변경하면 앱이 다시 로딩됨.
  - 워커(`XDWorldWorker.js`·`.wasm`)는 엔진과 짝이 맞아야 하는데 CDN에 없어서, 선택한 버전의 워커를 **워커 URL에서 받아 캐시**해 함께 바꾼다. URL에 `{version}`을 넣으면 버전별로 받는다(예: `https://사내서버/xdworld/{version}/XDWorldWorker.js`). `.wasm`은 같은 폴더에서 자동으로 받는다.
  - 찾는 순서: 앱 폴더 `worker/<버전>/` → 받아둔 캐시 → 워커 URL → 번들 워커. 현재 어떤 워커를 쓰는지 설정 패널에 표시되고, 번들 워커로 떨어지면 경고색으로 보인다.
